...
internal sealed class GlowManager : IDisposable
{
    ...
    private readonly Dictionary<IntPtr, OverlayEntry> _overlays = new();
    ...
    /// <summary>为目标窗口创建/更新光晕(已有则复用覆盖窗口,只刷新位置与绘制)。颜色由调用方按层级决定。</summary>
    ...
    /// <summary>移除并销毁指定窗口的光晕(不触发 TargetDestroyed)。</summary>
